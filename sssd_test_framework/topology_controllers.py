...
class GDMTopologyController(ProvisionedBackupTopologyController):
    ...
    def topology_setup(self, client: ClientHost, ipa: IPAHost, keycloak: KeycloakHost) -> None:
        ...
        ipa.kinit()
        ipa.conn.run(f"ipa idp-add keycloak --provider keycloak --base-url {keycloak.hostname}:8443/auth "
                     "--org master --client-id ipa_oidc_client --secret", input="IPA_Secret123")

        # Backup so we can restore to this state after each test
        super().topology_setup()

    def topology_teardown(self, client: ClientHost, ipa: IPAHost, keycloak: KeycloakHost):
        ...
        return super().topology_teardown()